...
class Field
{
   ...
   // const ADDRESS = 'address';
    ...
    function generate($elem, $ERROR = TRUE)
    {
        $this->elem = $elem;
        $this->wireModel = data_get($this->elem, FormConst::OPTIONS_WIREMODEL);
        $output = data_get($elem, FormConst::OUTPUT);
        ...
        if ($ERROR) {
            if (!is_null($errors)) {
                if ($errors !== false) {
                    ...
                    if ($errors) {
                        $mode = 'invalid';
                        $msg = collect(collect((array)$errors)->first())->implode('<br>');
                    }
                } else {
        ...
        switch ($output) {
            ...
            case self::LANG_SELECT:
                data_set($elem, FormConst::OUTPUT, self::SELECT);
            ...
            case self::TEXT_LANG:
                ...
                $num = $array->count();
                data_set($elem, FormConst::OUTPUT, self::TEXT);
            ...
            case self::BOOLEAN:
                ...
                break;
            case self::BOOLEAN2:
                data_set($elem, FormConst::OUTPUT, self::BOOLEAN);
                return $this->{__FUNCTION__}($elem);
                break;
            case self::RADIO:
            case self::CHECKBOX:
            ...
            case self::STATIC:
                switch ($output_edit) {
                    case self::BOOLEAN:
                        return $value ? '<i class="fa-regular text-success fa-circle-check"></i>' : '<i class="fa-regular text-danger fa-circle-xmark"></i>';
                        break;
                    case self::BOOLEAN2:
                        return $value ? '<i class="fa-solid text-success fa-check"></i>' : null;
                        break;
                }
                ...
                if (is_array($value)) {
                    $items = collect($value)->map(function ($v) {
                        return (string) Html::tag('li', $v, ['class' => 'list-group-item p-0 ps-1 pe-1 m-0']);
                    })->implode('');
                    return (string) Html::div($items, ['class' => 'list-group text-sm p-0 m-0']);
                }
                ...
                if (is_null($value)) {
                    return FormConst::STATIC_EMPTY;
                }

                return '[' . $output . '] da completare';
                break;
            case self::MSG:
